...
vector<char_node_t*> getDFS(char_node_t* root, char target)
{
	...
	while (!node_stack.empty())
	{
		...
		{
			std::cout << "visit: " << current->data << endl;
		}
		...
		{
			// if child node unvisited then mark visited and push onto stack
			if (!node->visited)
			{
				node->visited = true;
	...
	while (!node_stack.empty())
	{
		// add top node to front of path vector and then pop node off stack
		// if having trouble...Google (“add to front of vector c++”)
